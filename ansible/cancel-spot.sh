...
#aws ec2 describe-spot-instance-requests --filters "Name=tag:Name,Values=${comp}" --query "SpotInstanceRequests[*].[InstanceId]"
  ...
  SPOT_INSTANCE_ID=$(aws ec2 describe-spot-instance-requests --filters "Name=tag:Name,Values=${comp} Name=state,values=active" |jq ".SpotInstanceRequests[].SpotInstanceRequestId" |xargs)
